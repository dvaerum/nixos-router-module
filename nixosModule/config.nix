...
in (lib.mkIf cfg.enable {
      ...
      settings =
        {
          ...
          subnet4 = lib.forEach cfgSetDhcpServerInterfaceOnly (
            dhcp_interface_conf: let
              ...
              cidr = ipv4_fn.fromCidrString dhcp_server.address;

              subnet = "${cidr.network}/${builtins.toString cidr.prefix}";

              dhcpFirstIP =
                if dhcp_server.firstIP == null
                then ipv4_fn.nthAddress subnet 5
                else ipv4_fn.nthAddress subnet dhcp_server.firstIP;
            ...
            in
              {
                ...
                option-data =
                  # null -> advertise the router itself; [] -> advertise no DNS;
                  # a non-empty list -> advertise exactly those servers.
                  lib.lists.optional (dhcp_server.dns-servers != []) {
                    # code = 6;
                    name = "domain-name-servers";
                    csv-format = true;
                    data =
                      if dhcp_server.dns-servers == null
                      then cidr.address
                      else lib.strings.concatStringsSep ", " dhcp_server.dns-servers;
                  }
                  ++ lib.lists.optional (dhcp_server.default-route) {
                    # code = 3;
                    name = "routers";
                    data =
                      if dhcp_server.gateway == null
                      then cidr.address
                      else dhcp_server.gateway;
                  }
                    ...
                    data =
                      ...
                      (
                        _pos: dhcp_server_interface_conf: let
                          tmp_cidr = ipv4_fn.fromCidrString dhcp_server_interface_conf.dhcp.server.address;
                          tmp_subnet = "${tmp_cidr.network}/${builtins.toString tmp_cidr.prefix}";
                        in "${tmp_subnet} - ${cidr.address}"
                      )
                      (
                        lib.lists.filter (
                          dhcp_server_interface_conf:
                            dhcp_server_interface_conf.dhcp.server.address != dhcp_interface_conf.dhcp.server.address
                        )
                        cfgSetDhcpServerInterfaceOnly
                      );
                  }
  ...
  services.ntp = lib.attrsets.optionalAttrs (lib.length cfgSetDhcpServerInterfaceOnly > 0) {
    ...
    extraConfig =
      lib.strings.concatMapStringsSep "\n" (
        ...
        in "restrict ${cidr.network} mask ${cidr.netmask} nomodify notrap nopeer"
      )
      cfgSetDhcpServerInterfaceOnly;
  };
  ...
  services.resolved =
    ...
    // lib.mkIf cfg.dns-server.enable (
      if (lib.versionAtLeast lib.trivial.release "26.05")
      then {
        settings.Resolve = {
