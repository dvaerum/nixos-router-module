          ...
          in
          {
            ...
            option-data =
              ...
              lib.lists.optional (dhcp_server.dns-servers != [ ]) {
                ...
                csv-format = true;
                data =
                  if dhcp_server.dns-servers == null then
                    cidr.address
                  else
                    lib.strings.concatStringsSep ", " dhcp_server.dns-servers;
              }
              ++ lib.lists.optional (dhcp_server.default-route) {
                # code = 3;
                name = "routers";
                data = if dhcp_server.gateway == null then cidr.address else dhcp_server.gateway;
              }
